...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| COMPILER-015 | ~~`li rd, imm32` → `lui`+`ori` (or `addi` when it fits)~~ | done | m4 |
| COMPILER-016 | ~~`move rd, rs` → `add rd, rs, zero`~~ | done | m4 |
| COMPILER-017 | ~~`la rd, label` → `adr` or `lui`+`ori` as needed~~ | done | m4 |
...
| COMPILER-020 | ~~Load a YAP1 image into `yap_isa.Cpu` (skip header, copy payload to `load`, `PC=entry`) and run until `halt`~~ | done | m4 |
