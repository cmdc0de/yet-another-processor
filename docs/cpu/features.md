...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| CPU-039 | ~~Unconditional jump `j`: `PC ← (PC+4)[31:28] \|\| target26 \|\| 00`~~ | done | m5 |
| CPU-040 | ~~Jump-and-link `jal`: `ra ← PC+4`, then same target as `j`~~ | done | m5 |
| CPU-041 | ~~Jump register `jr`: `PC ← rs`~~ | done | m5 |
| CPU-042 | ~~Jump-and-link register `jalr`: `rd ← PC+4`, `PC ← rs`~~ | done | m5 |
| CPU-043 | ~~Conditional branch `bcc` on FLAGS (EQ, NE, signed and unsigned LT/GE and the rest of the ISA conds)~~ | done | m5 |
