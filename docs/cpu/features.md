...
| CPU-008 | ~~ALU and/or/xor/not; `Z N`; `C=V=0`~~ | implemented | m2 |
| CPU-009 | ~~Shifter sll/srl/sra; C=last bit shifted out; `V=0`~~ | implemented | m2 |
| CPU-010 | ~~Flags-only compare paths (`CMP`, `TEST`, `TEQ`)~~ | implemented | m2 |
| CPU-011 | ~~MUL low 32; DIV with ÷0 → `rd=0`, `Z=1`~~ | implemented | m2 |
| CPU-012 | ~~imm16 sign-extend, zero-extend, and LUI (`<<16`) onto the datapath~~ | implemented | m2 |
| CPU-013 | ~~PC-relative address adder (`ADR`)~~ | implemented | m2 |
...
| CPU-032 | ~~MOSFET bank and IC bank may take different cycle counts; the model exposes the split~~ | implemented | m2 |
