...
|----|---------|--------|-----------|
| CPU-001 | 32-bit datapath and 32-bit addresses | open | |
| CPU-002 | PC is a datapath register; sequential fetch is `PC+4` | open | |
| CPU-003 | Instruction register holds the current 32-bit instruction | open | |
| CPU-004 | Split GPR file: `r0`–`r2` wired, `r3`–`r15` MOSFET bank, `r16`–`r31` IC bank; one file to software | open | |
| CPU-005 | Writes to `r0`–`r2` discarded; ALU ops targeting them still update flags | open | |
| CPU-006 | Discrete FLAGS (`Z N C V`), not a GPR | open | |
| CPU-007 | ALU add/sub with ISA flag rules (add: C=carry; sub/cmp: C=borrow; V=signed overflow) | open | |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| CPU-018 | Microcode store and sequencer: each ISA insn is one or more cycles | open | |
| CPU-019 | Opcode / funct / Bcc cond dispatch to a microcode entry | open | |
| CPU-020 | Control word steers RF, ALU, shifter, PC, FLAGS, CSRs, memory | open | |
| CPU-021 | Fetch: aligned load into IR; unaligned fetch → CAUSE=8 | open | |
| CPU-022 | HALT stops the sequencer | open | |
| CPU-023 | Trap sequence: CAUSE, `PIE/PP←IE/P`, `P=1`, `IE=0`, EPC, `PC=0x80` | open | |
