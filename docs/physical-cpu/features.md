...
|----|---------|--------|-----------|
| PHYSICAL-CPU-001 | ~~KiCad sources under `hw/`~~ | implemented | m1 |

## 1-bit slices

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| PHYSICAL-CPU-002 | ~~1-bit static latch schematic (CELL latch: EN high = load)~~ | implemented | m1 |
| PHYSICAL-CPU-003 | ~~Latch ports: D, EN, Q, VDD, VSS~~ | implemented | m1 |
| PHYSICAL-CPU-004 | ~~Latch netlist in ngspice: EN=1 Q follows D; EN=0 Q holds~~ | implemented | m1 |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| PHYSICAL-CPU-008 | 32-bit adder: 32 copies of the adder bit, ripple `cin`/`cout` | open | |
| PHYSICAL-CPU-009 | 32-bit MOSFET register: 32 latches, one write-enable | open | |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| PHYSICAL-CPU-013 | Microcode ROM 256×64 and `uPC` sequencer (ICs allowed) | open | |
| PHYSICAL-CPU-014 | CPU memory port: addr, data, size, re, we (`docs/cpu/design.md`) | open | |
| PHYSICAL-CPU-015 | Integer ALU ops on the MOSFET datapath (add/sub/logic; shifts/mul/div may be IC) | open | |
