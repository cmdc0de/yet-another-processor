...
|----|---------|--------|-----------|
| MEMORY-BUS-001 | Bus cycles at CPU clock: command this cycle, data next (`LAT_MEM=1`) | open | |
| MEMORY-BUS-002 | CPU port maps to the bus: `addr[31:0]`, `size` ∈ {1,2,4}, `re`, `we` | open | |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| MEMORY-BUS-006 | Sources under `hw/bus/` | open | |
| MEMORY-BUS-007 | Write-then-read of one aligned word returns the stored value | open | |
| MEMORY-BUS-008 | Tests fail if the chosen simulator (`ngspice -b` and/or `iverilog`) is missing | open | |

## Rails

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| MEMORY-BUS-009 | SRAM and bus VDD are 3.3 V | open | |
