...
|----|---------|--------|-----------|
| MEMORY-BUS-001 | Bus cycles at CPU clock: command this cycle, data next (`LAT_MEM=1`) | claimed | m1 |
| MEMORY-BUS-002 | CPU port maps to the bus: `addr[31:0]`, `size` ∈ {1,2,4}, `re`, `we` | claimed | m1 |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| MEMORY-BUS-006 | Sources under `hw/bus/` | claimed | m1 |
| MEMORY-BUS-007 | Write-then-read of one aligned word returns the stored value | claimed | m1 |
| MEMORY-BUS-008 | Tests fail if the chosen simulator (`ngspice -b` and/or `iverilog`) is missing | claimed | m1 |

## Rails

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| MEMORY-BUS-009 | SRAM and bus VDD are 3.3 V | claimed | m1 |
