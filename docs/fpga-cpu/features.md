...
|----|---------|--------|-----------|
| FPGA-CPU-001 | ~~HDL sources under `emu/fpga-cpu/`~~ | implemented | m1 |
| FPGA-CPU-002 | ~~Testbench loads one YAP1: payload at `load`, `PC=entry`; header not in SRAM~~ | implemented | m1 |
| FPGA-CPU-003 | ~~Simulate until `halt` or a cycle cap; non-zero if it does not halt~~ | implemented | m1 |
| FPGA-CPU-004 | ~~Simulation builds and runs on Linux (dev host)~~ | implemented | m1 |

## Cycle contract

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| FPGA-CPU-005 | ~~One clock executes one 64-bit control word (`docs/cpu/design.md`)~~ | implemented | m1 |
| FPGA-CPU-006 | ~~Microcode ROM: 256 × 64, same field layout as `compiler.yap_cpu`~~ | implemented | m1 |
...
| FPGA-CPU-012 | ~~`halt` stops the clocked machine~~ | implemented | m1 |
