...
|----|---------|--------|-----------|
| FPGA-CPU-001 | ~~HDL sources under `emu/fpga-cpu/`~~ | done | m1 |
| FPGA-CPU-002 | ~~Testbench loads one YAP1: payload at `load`, `PC=entry`; header not in SRAM~~ | done | m1 |
| FPGA-CPU-003 | ~~Simulate until `halt` or a cycle cap; non-zero if it does not halt~~ | done | m1 |
| FPGA-CPU-004 | ~~Simulation builds and runs on Linux (dev host)~~ | done | m1 |
...
| FPGA-CPU-010 | ~~Load/store word/half/byte, LE; window and align traps~~ | done | m3 |
...
| FPGA-CPU-014 | ~~COP1 `mfc1`/`mtc1`; other COP1 → CAUSE=6~~ | implemented | m4 |

## Parity

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| FPGA-CPU-015 | ~~Existing ISA assembler tests (`compiler.tests.test_m*`) pass on the sim~~ | implemented | m4 |
| FPGA-CPU-016 | ~~`os/kernel.s` YAP1 reaches halt (exit 0) on the sim, as on `yap-emu`~~ | implemented | m4 |
