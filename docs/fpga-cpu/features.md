...
| FPGA-CPU-007 | Split RF: wired `r0`–`r2`, MOSFET `r3`–`r15` (`LAT=1`), IC `r16`–`r31` (`LAT=2`) | claimed | m2 |
| FPGA-CPU-008 | After halt (and after each retired insn in tests), architectural state matches `yap_cpu` | claimed | m2 |

## Execute

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| FPGA-CPU-009 | Integer ALU, shifts, MUL/DIV, immediates, ADR | claimed | m2 |
