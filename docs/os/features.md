...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| OS-004 | Kernel programs `UBASE`/`ULIMIT` before entering user | claimed | m2 |
| OS-005 | User fetch/load/store only inside that window | claimed | m2 |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| OS-007 | Kernel `ERET` to user entry with `P=0` | claimed | m2 |
| OS-008 | User `sp` set inside the user window | claimed | m2 |

## Traps

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| OS-009 | Trap handler saves enough user GPRs to resume (at least `a0`–`a4`, `ra`, `sp`) | claimed | m2 |
| OS-010 | `sys` (CAUSE=3) dispatches on `imm16` | claimed | m2 |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| OS-013 | Syscall ABI as ISA-049 (`a0`–`a3` in, `a0` return, `a4` errno) | claimed | m2 |
| OS-014 | `SYS_EXIT`: machine halts; `a4=0` | claimed | m2 |
