...
|----|---------|--------|-----------|
| OS-001 | ~~One YAP1: kernel at reset `PC=0`, trap handler at `0x80`~~ | implemented | m1 |
| OS-002 | ~~Image reaches halt (or `SYS_EXIT`) on `yap-emu`~~ | implemented | m1 |
| OS-003 | ~~Kernel stack in supervisor SRAM~~ | implemented | m1 |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| OS-018 | ~~Sources under `os/` assemble with `python3 -m compiler.asm`~~ | implemented | m1 |
| OS-019 | ~~Tests assemble a YAP1 and run it on `yap-emu`~~ | implemented | m1 |
