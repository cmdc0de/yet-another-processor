...
| OS-006 | User protection fault (CAUSE=2) is handled by the kernel | claimed | m3 |
...
| OS-011 | Align fault (CAUSE=8) is handled (exit or errno) | claimed | m3 |
| OS-012 | Unknown CAUSE: kernel panic path (halt, distinguishable dump) | claimed | m3 |
...
| OS-015 | `SYS_WRITE`: copy user buffer to a kernel log in SRAM (tests read it after halt) | claimed | m3 |
| OS-016 | Bad syscall number: `a4 ≠ 0`, user resumes | claimed | m3 |
| OS-017 | `SYS_WRITE` pointer outside the user window: errno, kernel memory intact | claimed | m3 |
