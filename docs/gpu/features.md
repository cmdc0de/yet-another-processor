...
|----|---------|--------|-----------|
| GPU-001 | HDL sources under `gpu/` | open | |
| GPU-002 | Simulation builds and runs on Linux (dev host) | open | |
| GPU-003 | Tests fail if the chosen simulator is missing | open | |

## Backbuffer

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| GPU-004 | One backbuffer in GPU memory (not CPU SRAM scanout) | open | |
| GPU-005 | Documented pixel format and at least one named resolution (`design.md`) | open | |
| GPU-006 | Test dump of the backbuffer (raw or PPM) matches a known fill | open | |
