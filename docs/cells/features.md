...
| CELL-012 | 1-bit full adder: `sum`, `cout` | claimed | m3 |

## Sequential

| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| CELL-013 | 1-bit static latch: holds while gated; takes a new bit when open | claimed | m3 |
...
| CELL-015 | Each cell in this catalog has a test schematic under `lt-spice/` | claimed | m3 |
| CELL-016 | Latch hold/load is shown in simulation (not only a static DC point) | claimed | m3 |
| CELL-017 | Adder checks include 0+0+cin0 and 1+1+cin1 | claimed | m3 |
