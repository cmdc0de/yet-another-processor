...
|----|---------|--------|-----------|
| EMU-RUST-001 | CLI takes a YAP1 path | open | |
| EMU-RUST-002 | Reject short files, bad `YAP1` magic, or `size != file_len - 16` | open | |
| EMU-RUST-003 | Copy payload to SRAM at `load`, set `PC=entry`; do not map the 16-byte header | open | |
| EMU-RUST-004 | Run until `halt` or a max-step cap (default 100000); non-zero exit if it does not halt | open | |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| EMU-RUST-006 | Builds and runs tests on Linux x86_64 | open | |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| EMU-RUST-011 | 32-bit addresses and little-endian SRAM | open | |
...
| EMU-RUST-023 | HALT sets halted; further steps are no-ops | open | |
