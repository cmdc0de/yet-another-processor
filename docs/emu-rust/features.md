...
| EMU-RUST-005 | ~~After the run, print PC, FLAGS, CSRs, and the 32 GPRs~~ | done | m5 |
...
| EMU-RUST-007 | Builds and runs tests on Linux aarch64 | claimed | m7 |
| EMU-RUST-008 | Builds and runs tests on Windows x86_64 | claimed | m7 |
| EMU-RUST-009 | Builds and runs tests on Windows aarch64 | claimed | m7 |
| EMU-RUST-010 | Builds and runs tests on macOS aarch64 | claimed | m7 |
...
| ID | Feature | Status | Milestone |
|----|---------|--------|-----------|
| EMU-RUST-024 | ~~User and supervisor; reset: supervisor, `IE=0`, `TE=0`, `PC=0`~~ | done | m5 |
| EMU-RUST-025 | ~~MFC0/MTC0; user may read FLAGS and STATUS; other user CSR access → CAUSE=7~~ | done | m5 |
| EMU-RUST-026 | ~~Trap entry: CAUSE, PIE/PP, `P=1`, `IE=0`, `PC=0x80`; EPC = faulting PC except SYS uses `PC+4`~~ | done | m5 |
| EMU-RUST-027 | ~~ERET is supervisor-only; restores IE/P from PIE/PP, `PC=EPC`~~ | done | m5 |
| EMU-RUST-028 | ~~One user window `[UBASE, ULIMIT)`; user fetch/load/store outside → CAUSE=2~~ | done | m5 |
| EMU-RUST-029 | ~~`IE` and a host-injected external IRQ → CAUSE=1~~ | done | m5 |
| EMU-RUST-030 | ~~`sys` → CAUSE=3~~ | done | m5 |
| EMU-RUST-031 | ~~Setting `TE=1` traps CAUSE=7 and leaves `TE=0`~~ | done | m5 |
| EMU-RUST-032 | ~~CAUSE codes 4 (translation miss) and 5 (page fault) exist; v1 does not generate them~~ | done | m5 |
